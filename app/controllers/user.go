...
func exchangeTokenByCode(code string) (string, error) {
	t := &oauth.Transport{Config: &oauth.Config{
		ClientId:     config.GetConfiguration().GithubClientId,
		ClientSecret: config.GetConfiguration().GithubClientSecret,
		RedirectURL:  config.GetConfiguration().GithubRedirectURL,
		TokenURL:     config.GetConfiguration().GithubTokenUrl,
		Scope:        config.GetConfiguration().GithubScope,
	}}
