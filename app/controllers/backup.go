...
func RestorePost(c *gin.Context) {
	...
	var err error
	if len(fileName) > 0 {
		fileUrl := config.GetConfiguration().QiniuFileServer + fileName
		...
		if err == nil {
			...
			if err == nil {
				data, err = helpers.Decrypt(data, config.GetConfiguration().BackupKey)
				if err == nil {
...
func Backup() error {
	var err error
	var u *url.URL
	u, err = url.Parse(config.GetConfiguration().DSN)
	if err == nil {
		if exists, _ := helpers.PathExists(u.Path); exists {
			...
			encryptData, err := helpers.Encrypt(data, config.GetConfiguration().BackupKey)
			if err != nil {
				seelog.Error(err)
				return err
			}

			conf.ACCESS_KEY = config.GetConfiguration().QiniuAccessKey
			conf.SECRET_KEY = config.GetConfiguration().QiniuSecretKey

			// 创建一个Client
			c := kodo.New(0, nil)
			// 设置上传的策略
			policy := &kodo.PutPolicy{
				Scope: config.GetConfiguration().QiniuBucket,
				//设置Token过期时间
				Expires: 3600,
			}
			// 生成一个上传token
