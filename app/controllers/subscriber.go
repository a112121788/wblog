...
func sendActiveEmail(subscriber *models.Subscriber) error {
	...
	err := sendMail(subscriber.Email, "[Wblog]邮箱验证", fmt.Sprintf("%s/active?sid=%s", config.GetConfiguration().Domain, signature))
	if err == nil {
		err = subscriber.Update()
	}
	return err
...
func GetUnSubcribeUrl(subscriber *models.Subscriber) (string, error) {
	...
	subscriber.Signature = signature
	err := subscriber.Update()
	return fmt.Sprintf("%s/unsubscribe?sid=%s", config.GetConfiguration().Domain, signature), err
}
